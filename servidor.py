...
def handle_client(client):  # Takes client socket as argument.
    ...
    mensagem = "%s entrou no chat!" % name
    broadcast(bytes(mensagem, "utf8"))
    clients[client] = name

    while True:
        mensagem = client.recv(BUFSIZ)
        if mensagem != bytes("{quit}", "utf8"):
...
def broadcast(mensagem, prefix=""):  # prefix is for name identification.
    """Broadcasts a message to all the clients."""

    for sock in clients:
        sock.send(bytes(prefix, "utf8") + mensagem)
